...
def generate_questions(level):
    ...
    for _ in range(5):
        if level == 1:
            ...
            def t1(): a,b=random.randint(1,10),random.randint(1,10); return f"Tom has {a} candies. He got {b} more. How many?", a+b
            def t2(): a,b=random.randint(5,15),random.randint(1,5); return f"Anna had {a} apples, gave away {b}. How many left?", a-b
            def t3(): a,b=random.randint(2,5),random.randint(2,5); return f"There are {a} bags, each with {b} toys. How many?", a*b
            def t4(): a=random.choice([6,8,10]);b=random.randint(1,a//2); return f"A pizza is cut into {a} slices. You ate {b}. Left?", a-b
            def t5(): a,b=random.randint(5,10),random.randint(1,4); return f"You have {a} pens, gave {b} to a friend. Left?", a-b
...
# --- Question Page ---
    ...
    if q_idx < len(questions):
        ...
        # Text input for integer-only answer
        user_input = st.text_input("Your Answer", key=f"answer_{q_idx}", placeholder="Enter a whole number")
        is_valid_int = user_input.strip().isdigit()

        if user_input and not is_valid_int:
            st.warning("Please enter only whole numbers (0, 1, 2, …).")
