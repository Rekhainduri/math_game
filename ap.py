...
def generate_questions(level):
    ...
    for _ in range(5):
        if level == 1:
            ...
            templates = [
                lambda: (f"Tom has {a:=random.randint(1, 10)} candies. He got {b:=random.randint(1, 10)} more. How many?", a + b),
                lambda: (f"Anna had {a:=random.randint(5, 15)} apples, gave away {b:=random.randint(1, 5)}. How many left?", a - b),
                lambda: (f"There are {a:=random.randint(2, 5)} bags, each with {b:=random.randint(2, 5)} toys. How many?", a * b),
                lambda: (f"A pizza is cut into {a:=random.choice([6, 8, 10])} slices. You ate {b:=random.randint(1, a//2)}. Left?", a - b),
                lambda: (f"You have {a:=random.randint(5, 10)} pens, gave {b:=random.randint(1, 4)} to a friend. Left?", a - b)
            ]
...
if st.session_state.page == 'welcome':
    ...
    if score >= 3:
        ...
        st.error("You need at least 3 correct answers to proceed. Try again!")
